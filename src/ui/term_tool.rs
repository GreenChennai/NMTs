...
fn draw_header(f: &mut Frame, app: &App, area: Rect) {
    ...
    let mut spans = vec![
        ...
        Span::styled(format!("{label} "), Style::default().fg(light).add_modifier(Modifier::BOLD)),
    ];
    ...
    for (i, v) in vendors.iter().enumerate() {
        let s = if i == app.term.vendor_idx {
            Span::styled(format!(" {}({}) ", v.label, v.vendor), Style::default().fg(Color::Black).bg(Color::Cyan))
        } else {
...
fn draw_scan(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    // 提示
    let mut lines = vec![
        Line::from(Span::styled(" 连接设备", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD))),
        Line::from(""),
        Line::from(Span::styled(format!(" 波特率：{}（S 切换）", app.term.baud), Style::default().fg(Color::White))),
        Line::from(""),
        Line::from(Span::styled(" Enter  扫描并连接", Style::default().fg(Color::Cyan))),
        Line::from(Span::styled(" S      切换波特率", Style::default().fg(Color::DarkGray))),
        Line::from(Span::styled(" ↑/↓    选择串口", Style::default().fg(Color::DarkGray))),
        Line::from(""),
    ...
    if let Some(s) = &app.term.status {
        ...
        lines.push(Line::from(Span::styled(format!(" {s}"), Style::default().fg(Color::Yellow))));
    }
...
fn draw_connected(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    if let Some(v) = app.vendor_db.vendors().get(app.term.vendor_idx) {
        ...
        scroll_list(
            f,
            chunks[0],
            Block::default().borders(Borders::ALL).title(" 命令模板（↑/↓ · Enter 发送） "),
            &items,
            selected,
    ...
    if app.term.input_mode {
        lines.push(Line::from(vec![
            ...
            Span::styled(format!("{}_", app.term.input), Style::default().fg(Color::Yellow)),
        ]));
    } else {
